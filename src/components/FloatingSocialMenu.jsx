...
export default function FloatingSocialMenu() {
  ...
  const socials = [
    ...
    },
    {
      ...
      bg: 'hover:bg-teal-500 hover:border-teal-500',
      text: 'text-teal-400',
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div ref={menuRef} className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      ...
      <div className={`flex flex-col gap-3 transition-all duration-300 origin-bottom-right ${
        ...
        {socials.map((social, idx) => (
          <a
            ...
            className={`group relative flex items-center justify-center w-12 h-12 rounded-full glass-panel border border-white/10 transition-all duration-300 shadow-[0_4px_20px_rgba(0,0,0,0.3)] hover:scale-110 hover:text-white ${social.bg} ${social.text}`}
            style={{ transitionDelay: isOpen ? `${(socials.length - idx) * 50}ms` : '0ms' }}
          >
            {social.icon}
            <span className="absolute right-14 px-3 py-1.5 bg-slate-800 text-white text-xs font-semibold rounded-lg opacity-0 translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 pointer-events-none whitespace-nowrap shadow-lg border border-white/10">
              {social.name}
            </span>
          </a>
        ))}
      ...
      <button
        ...
        className="relative flex items-center justify-center w-14 h-14 rounded-full bg-blue-600 text-white shadow-[0_8px_30px_rgba(37,99,235,0.5)] hover:bg-blue-500 hover:scale-105 hover:shadow-[0_8px_35px_rgba(37,99,235,0.7)] transition-all duration-300"
